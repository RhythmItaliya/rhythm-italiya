...
const Project = ({ item }) => {
  return (
    <div className="bg-[#082231]">
      <div
        key={item.id}
        data-aos={item.id % 2 == 0 ? "zoom-in-right" : "zoom-in-left"}
        className="w-80 lg:w-[365px] p-4 bg-[#142834] shadow-xl shadow-[#082231] mx-auto md:mx-0  rounded-md "
      >
        ...
        <div className="my-3 flex flex-row flex-wrap gap-2">
          {item.skills.map((skill, indx) => {
            return (
              <span
                key={skill + indx}
                className="bg-white/10 py-1 px-2 rounded-full text-xs font-popins text-gray-300"
              >
                {skill}
              </span>
            );
        ...
        <div>
          <h3 className="text-2xl font-bold font-popins text-gray-200">
            {item.name}
          </h3>
          <p className="mt-1 text-base font-medium font-popins text-gray-300">
            {item.desc}
          </p>
        </div>
        <div>
          <div className="flex flex-row gap-x-3 items-center mt-3">
            <a
              href={item.code}
              target="_blank"
              className="inline-flex overflow-hidden text-white bg-gray-900 rounded group cursor-pointer"
            >
